...
-- Tayfun HOCANIN having ÝLE ÇÖZÜMÜ:
SELECT  E.customer_id, first_name,E.last_name, D.order_id,
sum(case when A.category_name='Comfort Bicycles' then 1 else 0 end) is_comfort,
sum(case when A.category_name ='Children Bicycles' then 1 else 0 end) is_child

FROM production.categories A,production.products B,sales.order_items C,sales.orders D,sales.customers E
WHERE A.category_id=B.category_id AND B.product_id=C.product_id AND C.order_id=D.order_id AND D.customer_id=E.customer_id
Group by E.customer_id, first_name,E.last_name, D.order_id
having sum(case when A.category_name='Comfort Bicycles' then 1 else 0 end) >0 and
sum(case when A.category_name ='Children Bicycles' then 1 else 0 end) >0
